...
export function generateLocalServiceContent(input: ServiceContentInput) {
  ...
  const aboutVariants = [
    // Variant 0 — NK-specific, practical and direct
    [
      `NK Cab & Taxi runs local cab service in ${cityName} the way it should be run — you call ${BUSINESS.phone}, a real person confirms your booking in two minutes, and the driver shows up at your door before you expected. No app, no dynamic pricing, no mystery about what the fare will be. AC cars, verified drivers, and a rate that doesn't change whether it's 2 PM or 2 AM.`,
      `Our ${cityName} local packages work around actual usage: the 4-hour / 40-km Sedan package at ${prices.localPkgSedan} covers a hospital visit, bank run, or 2–3 errands comfortably. The 8-hour / 80-km package (Sedan ${prices.localPkgSuv}) handles a full day of travel — meetings across town, sightseeing, or having a car available for the day without committing to each trip separately.`,
      `What makes NK Cab different from app-based services in ${cityName}: the same driver stays with you the entire trip (no handoffs), rates are fixed before you confirm (no revision at pickup), and the car actually comes to your door (not 200 metres away on a busy road). Drivers know ${cityName}'s one-way streets, hospital gates, and which market lane to use.`,
      `All our ${cityName} local vehicles are AC, GPS-tracked, and cleaned between bookings. Swift Dzire and Honda Amaze for solo and small groups, Maruti Ertiga and Toyota Innova for families, Innova Crysta for premium day-bookings, and Tempo Traveller for groups of 10+. Driver is police-verified with a valid commercial licence.`,
      `To book a local taxi in ${cityName}: WhatsApp or call ${BUSINESS.phone}. Tell us your pickup address, your itinerary or first stop, your preferred vehicle, and the time. We confirm in under 2 minutes with driver name, vehicle number, and contact. Pay at trip end — Cash, UPI, or Card. No advance required.`,
      `NK Cab & Taxi covers every area of ${cityName} for local taxi pickup: ${landmarkList}${railway ? `, ${railway}` : ''}${airport ? `, ${airport}` : ''}. Corporate clients can ask about monthly packages with GST billing and 15–20% volume discount. Available 24/7, including Durga Puja, Eid, and New Year.`,
    ],
    // Variant 1 — Conversational, problem-first
    [
      `Getting around ${cityName} on a busy weekday — hospital appointment in the morning, school pickup in the afternoon, grocery run in the evening — is exactly the kind of situation our local taxi service was built for. NK Cab & Taxi runs local cab service in ${cityName} with AC cars, honest pricing, and no surge pricing whatsoever. Call ${BUSINESS.phone} and we'll have a driver confirmed in under 2 minutes.`,
      ...
      `NK Cab & Taxi covers all areas in ${cityName} for local taxi: ${landmarkList}${railway ? `, ${railway}` : ''}. Whether you live in a main locality or a residential colony on the outskirts — our drivers know the way. For office-goers who need daily morning-evening service, ask us about monthly packages with a 15–20% discount.`,
    ],
    // Variant 2 — Feature-led, trust signals
    [
      `NK Cab & Taxi has been providing local taxi service in ${cityName}, ${stateName} since ${BUSINESS.foundYear}. If you need a cab for a short trip or an all-day booking, our local packages have you covered. What makes the difference: police-verified drivers, AC cars, and the same fixed rate regardless of time of day or festival season. Call ${BUSINESS.phone} to book.`,
      `Local packages in ${cityName} — 4-hour/40-km (Sedan ${prices.localPkgSedan}): ideal for hospital visits, bank work, or 2–3 short stops. 8-hour/80-km (Sedan ${prices.localPkgSuv}): suitable for full-day city travel, corporate meetings, or guided sightseeing. SUV and Innova Crysta packages for larger groups. Extra km at ₹${prices.sedanRate}/km, extra hour at ₹150/hr.`,
      `Unlike app cabs, NK Cab & Taxi local taxi in ${cityName} doesn't cancel after accepting, doesn't surge during peak hours, and doesn't ask you to walk 200 metres to the 'closest point.' The driver comes to your door, waits while you finish an errand, and takes you where you need to go — without drama.`,
    ...
    // Variant 3 — Specific, factual, number-led
    [
      `Local taxi in ${cityName} from NK Cab & Taxi: fixed-rate AC cab service for city travel. Whether it's a morning commute, medical appointment, school run, or an all-day sightseeing trip — we're available round the clock. No app download needed. One call to ${BUSINESS.phone} and your cab is confirmed in 2 minutes.`,
    ...
    [
      ...
      `Festival days are when most cab apps fail in ${cityName} — drivers unavailable, surge pricing tripling fares, cancellations. NK Cab & Taxi was built for exactly those days. The same fixed fare on Durga Puja evening as on a quiet Tuesday morning. 24/7, 365 days. If you've had trouble with apps during peak times, give us a call: ${BUSINESS.phone}.`,
      `AC, GPS-tracked, and cleaned between each trip — that's the baseline for every NK Cab vehicle in ${cityName}. Beyond that: drivers who are police-verified, courteous, and genuinely experienced in ${cityName}'s roads. They wait while you're inside a hospital or bank without extra charges (within package time).`,
      ...
      `Serving all of ${cityName} including ${landmarkList}${airport ? `, ${airport}` : ''}. Daily commuters, corporate travelers, tourists, patients, students — they all use NK Cab in ${cityName}. Our local service is about reliability, not algorithms.`,
    ],
  ];
