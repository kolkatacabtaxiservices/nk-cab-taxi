...
export async function generateMetadata({ params }: { params: Promise<{ state: string; city: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: { title: `Return-Trip Car from ${city.name} | ${BUSINESS.name}`, type: 'website', siteName: BUSINESS.name, url: canonicalUrl, locale: 'en_IN', images: [{ url: `${BUSINESS.domain}/navbanner.webp`, width: 1200, height: 630, alt: `Return-trip car hire from ${city.name}` }] },
...
export default async function RoundTripPage({ params }: { params: Promise<{ state: string; city: string }> }) {
  ...
  return (
    <>
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(generateCitySubServiceSchema(city.name, state.name, state.slug, city.slug, 'Return-Trip Taxi Service', 'round-trip', `Hire a return-trip car from ${city.name} and keep one chauffeur for the whole journey. Sedan ${prices.sedanPerKm}, SUV ${prices.suvPerKm}. Multi-day trips at any hour.`, '12', '22')) }} />
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(generateFaqSchema(content.faqs)) }} />
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(generateBreadcrumbSchema([{ name: 'Home', url: BUSINESS.domain }, { name: state.name, url: `${BUSINESS.domain}/${state.slug}` }, { name: city.name, url: `${BUSINESS.domain}/${state.slug}/${city.slug}` }, { name: 'Return Trip', url: `${BUSINESS.domain}/${stateSlug}/${citySlug}/round-trip` }])) }} />

      <section className="relative text-white py-14 lg:py-20 overflow-hidden">
        <HeroBanner hideDots />
        <div className="relative z-10 max-w-7xl mx-auto px-4">
          <Breadcrumbs items={[{ name: state.name, href: `/${state.slug}` }, { name: city.name, href: `/${state.slug}/${city.slug}` }, { name: 'Return Trip', href: `/${state.slug}/${city.slug}/round-trip` }]} />
          <h1 className="text-3xl md:text-4xl lg:text-5xl font-extrabold mt-4 mb-4">Return-Trip Car from <span className="text-gradient">{city.name}</span> {prices.sedanPerKm}</h1>
          <p className="text-lg text-gray-300 max-w-3xl mb-6">A multi-day hire out of {city.name} with one chauffeur for the whole tour, and the driver lodges at your destination. Sedan, SUV, Tempo Traveller — service at any hour.</p>
          <div className="flex flex-wrap gap-3">
            ...
            <a href={`https://wa.me/${BUSINESS.whatsapp}?text=${encodeURIComponent(`Hello! I want a return-trip car from ${city.name}.`)}`} className="inline-flex items-center gap-2 px-6 py-3 bg-green-500 text-white font-bold rounded-full shadow-lg">💬 WhatsApp</a>
          </div>
        </div>
      </section>
      ...
      <section className="py-6 bg-white border-b border-gray-100"><div className="max-w-7xl mx-auto px-4"><div className="flex flex-wrap justify-center gap-6 text-sm">
        {[{ icon: <CheckCircle size={16} />, t: 'Same Driver Throughout' }, { icon: <Shield size={16} />, t: 'Screened Chauffeurs' }, { icon: <Clock size={16} />, t: 'Service 24x7' }, { icon: <Star size={16} />, t: 'No Surge Pricing' }].map((b, i) => (<div key={i} className="flex items-center gap-1.5 text-primary font-medium">{b.icon} {b.t}</div>))}
      </div></div></section>
      ...
      <section className="py-12 bg-white"><div className="max-w-7xl mx-auto px-4"><div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          <div><h2 className="text-2xl font-bold text-secondary mb-4">Return-Trip Hire from {city.name}, {state.name} — One Chauffeur for the Whole Tour</h2>
            ...
            <div className="mb-6 p-5 bg-amber-50 border border-amber-200 rounded-2xl flex flex-col md:flex-row gap-4 items-center">
              <div className="flex-1 w-full text-left">
                <h4 className="text-base font-bold text-secondary mb-3 flex items-center gap-2">
                  ⚡ Quick Facts — {city.name} Return-Trip Taxi
                </h4>
                <dl className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
                  ...
                  <div className="bg-white rounded-xl p-3 text-center border border-amber-100">
                    <dt className="text-xs text-gray-400 mb-1">Night Allowance</dt>
                    <dd className="font-bold text-primary">₹300/allowance</dd>
                  </div>
                </dl>
                <p className="text-xs text-gray-500 mt-3">
                  📍 Great for: extended family trips, weekend breaks out of town, pilgrimage circuits and work travel spread over several days.
                </p>
                <p className="text-xs text-gray-500 mt-1">
                  📞 Always-on support: <a href={`tel:${BUSINESS.phone}`} className="text-primary font-semibold hover:underline">{BUSINESS.phone}</a> | Driver lodging is sorted for you.
                </p>
              </div>
              <div className="relative w-full md:w-40 h-28 rounded-xl overflow-hidden shrink-0 border border-amber-200 shadow-sm">
                <Image
                  src="/navbanner.webp"
                  alt={`Return-trip car service out of ${city.name} - air-conditioned sedan and SUV booking`}
                  fill
          ...
          <div><h3 className="text-xl font-bold text-secondary mb-4">Return-Trip Rates from {city.name}</h3>
            <div className="overflow-x-auto rounded-xl border border-gray-100 shadow-sm"><table className="w-full border-collapse bg-white">
              <thead><tr className="bg-secondary text-white"><th className="px-4 py-3 text-left text-sm">Car</th><th className="px-4 py-3 text-center text-sm">Rate/km</th><th className="px-4 py-3 text-center text-sm">Minimum/day</th><th className="px-4 py-3 text-center text-sm">Seats</th></tr></thead>
          ...
          <div><h3 className="text-xl font-bold text-secondary mb-4">Why Book a Return Trip from {city.name}?</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {['Your chauffeur remains with you; no fresh booking at the far end', 'The same car covers city sightseeing and the journey home', 'Lengths run from two days up to a full month', 'Stable rates, day or night, with no peak markups', 'Chauffeur stay is part of the package', 'Cancel for free up to four hours prior', 'Air-conditioned cars with GPS monitoring', 'Booking confirmed on WhatsApp within two minutes'].map((item, i) => (<div key={i} className="flex items-start gap-2 text-sm text-gray-600"><CheckCircle size={16} className="text-green-500 shrink-0 mt-0.5" />{item}</div>))}
            </div>
          </div>
        </div>
        <div className="lg:sticky lg:top-24 lg:self-start space-y-4">
          <BookingForm defaultFrom={city.name} compact />
          <div className="p-4 bg-secondary rounded-xl text-white text-center"><p className="text-sm text-gray-300 mb-2">Return-Trip Support</p><a href={`tel:${BUSINESS.phone}`} className="text-xl font-bold hover:text-primary transition-colors">📞 {BUSINESS.phone}</a></div>
        </div>
      </div></div></section>

      {routes.length > 0 && (<section className="py-12 bg-gray-50"><div className="max-w-7xl mx-auto px-4">
        <h2 className="text-2xl font-bold text-secondary mb-6">Popular Return-Trip Routes out of <span className="text-primary">{city.name}</span></h2>
      ...
      <section className="py-12 bg-gradient-to-r from-primary to-amber-500 text-white text-center"><div className="max-w-4xl mx-auto px-4">
        <h2 className="text-2xl md:text-3xl font-bold mb-3">Reserve a Return Trip from {city.name}!</h2>
        <p className="text-white/90 mb-6">{prices.sedanPerKm} | Same Driver | Multi-Day | Any Hour</p>
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          ...
          <a href={`https://wa.me/${BUSINESS.whatsapp}?text=${encodeURIComponent(`Hello! Return trip from ${city.name}.`)}`} className="inline-flex items-center justify-center gap-2 px-8 py-4 bg-green-500 text-white font-bold rounded-full text-lg shadow-lg hover:scale-105 transition-all">💬 WhatsApp</a>
        </div>
      </div></section>
    </>
  );
