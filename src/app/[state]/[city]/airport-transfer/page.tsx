...
export async function generateMetadata({ params }: { params: Promise<{ state: string; city: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: { title: `Airport Taxi ${city.name} from ${metaPrices.airportSedan} | ${BUSINESS.name}`, type: 'website', siteName: BUSINESS.name, url: canonicalUrl, locale: 'en_IN', images: [{ url: `${BUSINESS.domain}/navbanner.webp`, width: 1200, height: 630, alt: `Airport taxi in ${city.name}` }] },
...
export default async function AirportTransferPage({ params }: { params: Promise<{ state: string; city: string }> }) {
  ...
  return (
    <>
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(generateCitySubServiceSchema(city.name, state.name, state.slug, city.slug, 'Airport Taxi Service', 'airport-transfer', `Reliable airport transfer service to and from ${airportName}, ${city.name} with live flight tracking, terminal reception and round-the-clock operation. Sedans from ₹1200.`, '800', '3000')) }} />
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(generateFaqSchema(content.faqs)) }} />
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(generateBreadcrumbSchema([{ name: 'Home', url: BUSINESS.domain }, { name: state.name, url: `${BUSINESS.domain}/${state.slug}` }, { name: city.name, url: `${BUSINESS.domain}/${state.slug}/${city.slug}` }, { name: 'Airport Taxi', url: `${BUSINESS.domain}/${stateSlug}/${citySlug}/airport-transfer` }])) }} />

      <section className="relative text-white py-14 lg:py-20 overflow-hidden">
        <HeroBanner hideDots />
        <div className="relative z-10 max-w-7xl mx-auto px-4">
          <Breadcrumbs items={[{ name: state.name, href: `/${state.slug}` }, { name: city.name, href: `/${state.slug}/${city.slug}` }, { name: 'Airport Taxi', href: `/${state.slug}/${city.slug}/airport-transfer` }]} />
          <h1 className="text-3xl md:text-4xl lg:text-5xl font-extrabold mt-4 mb-4">Airport Taxi in <span className="text-gradient">{city.name}</span> — {prices.airportSedan} Fixed</h1>
          <p className="text-lg text-gray-300 max-w-3xl mb-6">Rides to and from {airportName} with live flight tracking, a name-board welcome at the terminal and help with luggage. One fixed price, never a surge, at any hour.</p>
          <div className="flex flex-wrap gap-3">
            ...
            <a href={`https://wa.me/${BUSINESS.whatsapp}?text=${encodeURIComponent(`Hello! I want an airport taxi in ${city.name}.`)}`} className="inline-flex items-center gap-2 px-6 py-3 bg-green-500 text-white font-bold rounded-full shadow-lg">💬 WhatsApp</a>
          </div>
        </div>
      </section>
      ...
      <section className="py-6 bg-white border-b border-gray-100"><div className="max-w-7xl mx-auto px-4"><div className="flex flex-wrap justify-center gap-6 text-sm">
        {[{ icon: <Plane size={16} />, t: 'Live Flight Updates' }, { icon: <Shield size={16} />, t: 'Meet & Greet' }, { icon: <Clock size={16} />, t: 'Service 24x7' }, { icon: <Star size={16} />, t: 'Fixed Price, No Surge' }].map((b, i) => (<div key={i} className="flex items-center gap-1.5 text-primary font-medium">{b.icon} {b.t}</div>))}
      </div></div></section>
      ...
      <section className="py-12 bg-white"><div className="max-w-7xl mx-auto px-4"><div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          <div><h2 className="text-2xl font-bold text-secondary mb-4">Airport Transfer Service in {city.name} — {airportName} Rides Both Ways</h2>
            ...
            <div className="mb-6 p-5 bg-amber-50 border border-amber-200 rounded-2xl flex flex-col md:flex-row gap-4 items-center">
              <div className="flex-1 w-full text-left">
                <h4 className="text-base font-bold text-secondary mb-3 flex items-center gap-2">
                  ⚡ Quick Facts — {city.name} Airport Cab
                </h4>
                <dl className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
                  <div className="bg-white rounded-xl p-3 text-center border border-amber-100">
                    <dt className="text-xs text-gray-400 mb-1">Sedan Price</dt>
                    <dd className="font-bold text-secondary">₹{prices.airportSedan}</dd>
                  </div>
                  <div className="bg-white rounded-xl p-3 text-center border border-amber-100">
                    <dt className="text-xs text-gray-400 mb-1">SUV Price</dt>
                    <dd className="font-bold text-secondary">₹{prices.airportSuv || '2,500'}</dd>
                  </div>
                  <div className="bg-white rounded-xl p-3 text-center border border-amber-100">
                    <dt className="text-xs text-gray-400 mb-1">Delay Waiting</dt>
                    <dd className="font-bold text-primary">Free (No Limit)</dd>
                  </div>
                  <div className="bg-white rounded-xl p-3 text-center border border-amber-100">
                    <dt className="text-xs text-gray-400 mb-1">Tolls and Parking</dt>
                    <dd className="font-bold text-primary">Included</dd>
                  </div>
                </dl>
                <p className="text-xs text-gray-500 mt-3">
                  📍 Coverage: direct rides to and from {airportName} with live flight monitoring and a terminal welcome.
                </p>
                <p className="text-xs text-gray-500 mt-1">
                  📞 Always-on support: <a href={`tel:${BUSINESS.phone}`} className="text-primary font-semibold hover:underline">{BUSINESS.phone}</a> | Punctual, dependable airport transfers.
                </p>
              </div>
              <div className="relative w-full md:w-40 h-28 rounded-xl overflow-hidden shrink-0 border border-amber-200 shadow-sm">
                <Image
                  src="/navbanner.webp"
                  alt={`Airport transfer taxi service in ${city.name} - air-conditioned sedan and SUV booking`}
                  fill
          ...
          <div><h3 className="text-xl font-bold text-secondary mb-4">Airport Taxi Rates in {city.name}</h3>
            <div className="overflow-x-auto rounded-xl border border-gray-100 shadow-sm"><table className="w-full border-collapse bg-white">
              <thead><tr className="bg-secondary text-white"><th className="px-4 py-3 text-left text-sm">Car</th><th className="px-4 py-3 text-center text-sm">Airport Price</th><th className="px-4 py-3 text-center text-sm">Seats</th></tr></thead>
            ...
            <p className="text-xs text-gray-400 mt-2">* Fixed fare within the airport zone. Beyond that, ₹{vehicles[0]?.pricePerKm || 12} per km. Tolls and parking are built in.</p>
          </div>
          <div><h3 className="text-xl font-bold text-secondary mb-4">Airport Taxi Features in {city.name}</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {['Live flight monitoring; we reschedule if your flight runs late', 'A name board greeting at the arrivals gate', 'Up to 45 minutes of free waiting when flights are delayed', 'The chauffeur lends a hand with your baggage', 'One set price — no meter, no peak fees, nothing hidden', 'Open at all hours, even 3 AM arrivals', 'Air-conditioned cars, GPS monitored and cleaned between trips', 'Business airport plans with GST invoices'].map((item, i) => (<div key={i} className="flex items-start gap-2 text-sm text-gray-600"><CheckCircle size={16} className="text-green-500 shrink-0 mt-0.5" />{item}</div>))}
            </div>
          </div>
        </div>
        <div className="lg:sticky lg:top-24 lg:self-start space-y-4">
          <BookingForm defaultFrom={city.name} compact />
          <div className="p-4 bg-secondary rounded-xl text-white text-center"><p className="text-sm text-gray-300 mb-2">Airport Taxi Support</p><a href={`tel:${BUSINESS.phone}`} className="text-xl font-bold hover:text-primary transition-colors">📞 {BUSINESS.phone}</a></div>
        </div>
      </div></div></section>
      ...
      <section className="py-12 bg-gradient-to-r from-primary to-amber-500 text-white text-center"><div className="max-w-4xl mx-auto px-4">
        <h2 className="text-2xl md:text-3xl font-bold mb-3">Reserve an Airport Taxi in {city.name}!</h2>
        <p className="text-white/90 mb-6">Sedan {prices.airportSedan} | Live Tracking | Terminal Welcome | Any Hour</p>
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          ...
          <a href={`https://wa.me/${BUSINESS.whatsapp}?text=${encodeURIComponent(`Hello! Airport taxi in ${city.name}.`)}`} className="inline-flex items-center justify-center gap-2 px-8 py-4 bg-green-500 text-white font-bold rounded-full text-lg shadow-lg hover:scale-105 transition-all">💬 WhatsApp</a>
        </div>
      </div></section>
    </>
  );
